...
export class SettingsScreen extends React.Component {
    ...
    saveAndClose = ()=>{
        ...
        if (deviceIP && validIP) {
            storage.writeStoredSettings(JSON.parse('{"DeviceIP":"'+deviceIP+'"}')).then(()=>{
    ...
    render() {
      ...
      return (
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 20, }}>
          <Text style={{ fontSize: 30 }}>Device IP:</Text>
          <TextInput 
            style={{ height: 40, width: '100%', borderColor: 'gray', borderWidth: 1, margin: 5, paddingHorizontal: 10, backgroundColor: validIP ? '#b8f2cd':'#f2f2b8' }}
