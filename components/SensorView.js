...
export class SensorView extends Component {
    ...
    componentDidUpdate() {
        ...
        if (this.props.ipAddress!=='null' && this.props.ipAddress!==this.deviceIP){
    ...
    componentDidMount()
    {
        this.device.emitter.addListener('connection-state', (args)=>{
            this.setState({connected: args.server_status==="connected" ? true:false});
        });

        this.device.emitter.addListener('sensor-state', (args)=>{
            console.log(args)
            this.setState({connected: true, temperature: args.temperature, humidity: args.humidity, light: args.relay});
        });
        this.setState({connStatus: "Connecting to "+this.deviceIP})
        ...
        if (!connected){
            return(
                <View style={this.styles.connectionScreen}>
                    <Image source={require('../assets/connecting.png')} style={this.styles.connectionImage}></Image>
                    <Text style={this.styles.connectionText}>{connStatus}</Text>
                </View>
            )
        }
        return (
            <ImageBackground source={light ==="True" ? require('../assets/light-bulbs.jpg'):require('../assets/light-bulbs-off.jpg')} 
            style={this.styles.mainBackground}>
                <View style={this.styles.container}>
                    <View style={this.styles.sensorBlock}>
                        <View style={{margin: 5}}>
                            <Image source={require('../assets/humidity.png')} style={{ width:40,height: 40}}></Image>
                            <Text style={{fontSize:30}}>{humidity}%</Text>
                        </View>
                        <View style={{margin: 5}}>
                            <Image source={require('../assets/temperature.png')} style={{ width:40,height: 40}}></Image>
                            <Text style={{fontSize:30}}>{temperature}C</Text>
                        </View>
                        <View style={this.styles.separator} />
                    </View>
                    <Button title={light ==="True" ? "Swich OFF":"Switch ON"} onPress={this.switchRelay}/>
                </View>
            </ImageBackground>
          );
    }
