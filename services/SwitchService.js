...
export class SwitchService {
    ...
    connect = (deviceIP) => {
        ...
        if (this.connType===connectWith.SOCKET){
            ...
            this.socket.onclose = e => {
                ...
                this.connected=false;
                console.log(
                    `Socket is closed. Reconnect will be attempted in ${CONNECT_RETRY_INTERVAL_MS/1000} seconds.`
                );
                this.connectInterval = setTimeout(this.check, CONNECT_RETRY_INTERVAL_MS);
            };
    ...
    processSwitchData = function (dataRaw) {
        ...
        if (data.msg_type==='state'){
            ...
            this.emitter.emit('sensor-state', { temperature: data.temperature,
